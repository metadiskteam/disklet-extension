...
export async function getAddress(chain: Chain = 'mvc', path?: string): Promise<string> {
  ...
  try {
    ...
    const concatPath = `${rootPath}/${derivedPath(path)}`
...
export async function getPrivateKey(chain: Chain = 'mvc', path?: string) {
  const network = await getNetwork()
  const mnemonic = await getCurrentAccount().then((account) => account!.mnemonic)

  let _path = path
  if(!_path){
    _path = await getAccountProperty(chain, 'path')
  }
  else{
    const rootPath = await getMvcRootPath()
    _path = `${rootPath}/${derivedPath(_path)}`
  }

  return derivePrivateKey({ mnemonic, chain, network, path:_path })
}
